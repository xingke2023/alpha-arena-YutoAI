...
export default function AnalyticsPanel() {
  ...
  return (
    <div className="space-y-3">
      ...
      {isLoading ? <div className="text-xs" style={{ color: 'var(--muted-text)' }}>加载中…</div> : null}
    </div>
  );
}
...
function Kpi({ label, value }: { label: string; value?: string }) {
  return (
    <div className={`rounded-md border p-3`} style={{ background: 'var(--panel-bg)', borderColor: 'var(--panel-border)' }}>
      <div style={{ color: 'var(--muted-text)' }}>{label}</div>
      <div className={`mt-1 text-sm`} style={{ color: 'var(--foreground)' }}>{value ?? "—"}</div>
    </div>
  );
}
...
function Block({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className={`rounded-md border`} style={{ borderColor: 'var(--panel-border)' }}>
      <div className={`border-b px-3 py-2 text-xs`} style={{ borderColor: 'var(--panel-border)', color: 'var(--muted-text)' }}>{title}</div>
...
function MiniTable({ rows }: { rows: any[] }) {
  if (!rows?.length) return <div className="text-xs" style={{ color: 'var(--muted-text)' }}>暂无数据</div>;
      ...
      <table className="w-full text-left text-[11px]">
        ...
        <tbody style={{ color: 'var(--foreground)' }}>
          {rows.slice(0, 20).map((r, idx) => (
            <tr key={idx} className={`border-b`} style={{ borderColor: 'color-mix(in oklab, var(--panel-border) 50%, transparent)' }}>
              {cols.map((c) => (
