...
try { if (process.env.OPENAI_EXTRA_HEADERS) EXTRA_HEADERS = JSON.parse(process.env.OPENAI_EXTRA_HEADERS); } catch {}
let EXTRA_BODY: any = undefined;
try { if (process.env.OPENAI_EXTRA_BODY) EXTRA_BODY = JSON.parse(process.env.OPENAI_EXTRA_BODY); } catch {}
...
function hashKey(s: string) { return createHash('sha256').update(s).digest('base64url').slice(0, 16); }
function getCached(key: string): string | undefined {
  const e = memCache.get(key);
  if (!e) return undefined;
  if (now() - e.ts > TTL_MS) { memCache.delete(key); return undefined; }
  return e.v;
}
...
function sanitizeHeaders(h: Record<string, string>) {
  ...
  for (const [k, v] of Object.entries(h)) {
    ...
    if (low.includes('authorization') || low.includes('api-key') || low.includes('token') || low.includes('secret') || k === AUTH_HEADER) {
      out[k] = redact(v);
    } else {
      out[k] = v;
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if ((origin && origin !== selfOrigin) || (referer && !referer.startsWith(selfOrigin))) {
    ...
    if (cached) return NextResponse.json({ translation: cached, cached: true }, { headers: { 'x-cache': 'hit' }});
    ...
    const hasExplicitPath = /(chat\/completions|responses)(\b|\/|$)/.test(trimmed);
    const effectiveUrl = hasExplicitPath ? trimmed : `${trimmed}/chat/completions`;
    ...
    const body = EXTRA_BODY ? { ...baseBody, ...EXTRA_BODY, extra_body: EXTRA_BODY } : baseBody;
    if (IS_DEV) {
      const keys = Object.keys(body || {});
      console.log('[translate] body keys', keys, 'has routing?', 'model_routing_config' in body, 'len', JSON.stringify(body).length);
    }
    ...
    if (!('Authorization' in headers)) headers['Authorization'] = `Bearer ${OPENAI_API_KEY}`;
    if (OPENAI_ORG) headers["OpenAI-Organization"] = OPENAI_ORG;
    if (OPENAI_PROJECT) headers["OpenAI-Project"] = OPENAI_PROJECT;

    if (IS_DEV) {
      console.log('[translate] upstream', effectiveUrl, 'headers', sanitizeHeaders(headers), 'useResponses', useResponses, 'model', MODEL);
    }

    // Try SDK first to match Python client's behavior; fall back to fetch if SDK call fails
    let translation = "";
    try {
      const defaultHeaders: Record<string, string> = { accept: 'application/json', ...EXTRA_HEADERS };
      if (AUTH_HEADER && (AUTH_HEADER !== 'Authorization' || AUTH_SCHEME === '')) {
        defaultHeaders[AUTH_HEADER] = AUTH_SCHEME === '' ? OPENAI_API_KEY! : `${AUTH_SCHEME} ${OPENAI_API_KEY}`.trim();
      }
      ...
      const req: any = useResponses
        ? { model: MODEL, input: prompt }
        : { model: MODEL, messages: [ { role: "user", content: prompt } ] };
      if (EXTRA_BODY) { req.extra_body = EXTRA_BODY; Object.assign(req, EXTRA_BODY); }
      if (useResponses) {
        const r = await client.responses.create(req);
        translation = (r as any)?.output_text || (r as any)?.choices?.[0]?.message?.content || "";
      } else {
        try {
          const r = await client.chat.completions.create(req);
          translation = (r as any)?.choices?.[0]?.message?.content || "";
        } catch (e: any) {
          if (IS_DEV) console.log('[translate][sdk] chat  error', String(e?.message || e).slice(0, 200));
          // Fallback to responses endpoint via SDK if available
          try {
            const r2 = await client.responses.create({ model: MODEL, input: prompt, ...(EXTRA_BODY ? EXTRA_BODY : {}) } as any);
            translation = (r2 as any)?.output_text || (r2 as any)?.choices?.[0]?.message?.content || "";
          } catch (e2: any) {
            if (IS_DEV) console.log('[translate][sdk] resp error', String(e2?.message || e2).slice(0, 200));
          }
        }
      }
    ...
    if (!translation) {
      ...
      if (!upstream.ok || !ctype2.includes('application/json')) {
        ...
        if (!useResponses && upstream.status === 403) {
          const respUrl = tryUrl.replace(/\/chat\/completions(\b|\/|$)/, '/responses');
          const respBody = { model: MODEL, input: prompt, temperature: 0.2, ...(EXTRA_BODY ? EXTRA_BODY : {}) } as any;
          if (IS_DEV) console.log('[translate][fetch] fallback to responses', respUrl);
          upstream = await fetch(respUrl, { method: 'POST', headers, body: JSON.stringify(respBody) });
        }
      }
      const ctype3 = upstream.headers.get('content-type') || '';
      if (!upstream.ok || !ctype3.includes('application/json')) {
        const t = await upstream.text().catch(() => '');
        return NextResponse.json({ translation: null, disabled: true, error: t ? t.slice(0, 300) : `OpenAI ${upstream.status}`, meta: { url: effectiveUrl, auth_header: AUTH_HEADER, scheme: AUTH_SCHEME || '(none)' } }, { headers: { 'x-cache': 'miss' }});
      }
      const data2 = await upstream.json().catch(() => ({}));
      translation = useResponses
        ? (data2?.output_text || data2?.choices?.[0]?.message?.content || "").trim?.() ?? ""
        : (data2?.choices?.[0]?.message?.content || "").trim?.() ?? "";
    }
    if (translation) setCached(cacheKey, translation);
    return NextResponse.json({ translation }, { headers: { 'x-cache': 'store' }});
  } catch (e: any) {
    return NextResponse.json({ translation: null, disabled: true, error: String(e?.message || e), meta: { url: process.env.OPENAI_API_URL || '', auth_header: AUTH_HEADER, scheme: AUTH_SCHEME || '(none)' } }, { headers: { 'x-cache': 'error' }});
  }
}
